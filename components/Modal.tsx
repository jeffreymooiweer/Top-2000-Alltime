  ...
  return (
    ...
    >
        ...
        <div className="flex-1 min-w-0">
            <h4 className="text-gray-900 font-bold text-sm truncate group-hover:text-[#d00018] transition">{song.title}</h4>
...
const Modal: React.FC<ModalProps> = ({ song, onClose, otherSongsByArtist, onSelectSong, onNext, onPrevious, hasNext, hasPrevious }) => {
        ...
        <div className="relative bg-[#d00018] text-white p-6 md:p-8 flex flex-col md:flex-row gap-6 items-start md:items-end">
          ...
          >
            ...
            <div className="flex-1 min-w-0 pb-2 mr-8 md:mr-10">
              <h2 className="text-3xl md:text-5xl font-bold brand-font truncate leading-tight">{song.title}</h2>
              ...
              <div className="flex flex-wrap gap-2 mt-3 text-sm font-semibold opacity-90">
                 <span className="bg-white/20 px-2 py-1 rounded">Allertijden Rank #{song.allTimeRank}</span>
                 <span className="bg-white/20 px-2 py-1 rounded">{song.releaseYear > 0 ? song.releaseYear : 'N/A'}</span>
                 <span className="bg-white/20 px-2 py-1 rounded">{song.totalScore?.toLocaleString()} punten</span>
              </div>
            </div>
        </div>
            ...
            {activeTab === 'overview' ? (
                <div className="space-y-8 animate-fade-in">
                    ...
                    <h3 className="text-xl font-bold text-gray-900 mb-4 flex items-center gap-2 brand-font uppercase">
                        <svg className="w-5 h-5 text-[#d00018]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z" /></svg>
                    ...
                    <section>
                        <h3 className="text-xl font-bold text-gray-900 mb-2 flex items-center gap-2 brand-font uppercase">
                            <svg className="w-5 h-5 text-[#d00018]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                            Wist je dat?
                        </h3>
                        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm text-gray-700 leading-relaxed relative">
                            ...
                            <p className="text-[10px] text-gray-400 mt-4 text-right">Powered by Google Gemini</p>
                        </div>
                    </section>
                    ...
                    <section>
                        <h3 className="text-xl font-bold text-gray-900 mb-4 brand-font uppercase">Meer van {song.artist} in de lijst</h3>
                ...
                <div className="animate-fade-in bg-white p-6 md:p-10 rounded-xl shadow-inner min-h-[300px]">
                    {loadingLyrics ? (
                         ...
                         <div className="text-center">
                             <h4 className="font-bold text-lg mb-6 brand-font uppercase text-gray-400">Songtekst</h4>
                             <p className="whitespace-pre-line text-gray-800 leading-8 font-medium text-lg font-serif">
                                {lyrics}
                             </p>
                             <p className="mt-8 text-xs text-gray-400">Songteksten aangeboden door lyrics.ovh</p>
                         </div>
                    )}
                </div>
            )}
